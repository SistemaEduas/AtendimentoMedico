...
export default function EditarMedicoPage({ params }: { params: { id: string } }) {
  ...
  useEffect(() => {
    async function carregarMedico() {
      const { data, error } = await supabase.from("medicos").select("*").eq("id", params.id).single()
  ...
  async function onSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    const { error } = await supabase.from("medicos").update(formData).eq("id", params.id)
  ...
  return (
    <div className="p-6 max-w-3xl mx-auto">
      <div className="flex items-center gap-4 mb-6">
        ...
        <h1 className="text-3xl font-bold">Editar Médico</h1>
      </div>
